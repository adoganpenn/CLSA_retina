...
class AgeGlaucomaModelContractTests(unittest.TestCase):
    ...
    def test_explainability_is_source_specific_and_reproduction_gated(self) -> None:
        ...
        self.assertIn("checkpoint_cache_path", source)
        self.assertIn("temporary_checkpoint.replace", source)
        self.assertIn("sha256_file", source)
        self.assertIn('os.environ.pop("HF_TOKEN", None)', source)
        self.assertIn("The RETFound checkpoint is gated", source)
        self.assertIn('%pip install "timm==1.0.28"', source)
        self.assertIn("load_zeiss_retfound_model", source)
        self.assertIn("Reloaded source-specific age/glaucoma module", source)
        self.assertIn("source_model = zeiss_model", source)
        self.assertIn("source_model = clsa_model", source)
        self.assertIn("embedding_reproduction_summary.csv", source)
        self.assertIn("source_specific_attribution_maps.npz", source)
